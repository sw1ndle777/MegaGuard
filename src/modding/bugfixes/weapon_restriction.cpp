    ...
    if (eventType == 257)
    {
        switch (dialogId)
        {
            case 103053:
            {
                ...
                auto* room = reinterpret_cast<CRoom*>(GetCRoom());
                if (!room) break;
    ...
    if (!checkVendorLobby && eventType == 257)
    {
        switch (dialogId)
        {
            case 101069:
            {
				...
				if (!unitContainer) break;
                if (room->m_kPlayer[0])
                {
					auto hostUniqueId = room->m_kPlayer[0]->m_kRoomInfo->uniqueId.data;
					auto localUniqueId = unitContainer->m_pGamePlayer[5]->m_pExPlayer->m_kRoomInfo->uniqueId.data;
					if (hostUniqueId == localUniqueId)
                    {
