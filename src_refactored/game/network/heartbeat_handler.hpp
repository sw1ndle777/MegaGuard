...
// Mapping of 21 DetectionFlag bits to non-sequential positions in 128-bit field.
// Gaps are filled with noise bits — if a cheater zeros the whole field, the
// noise won't match and the server will know.
inline constexpr u8 kDetectionBitMap[21] = {
...
struct HeartbeatResponse {
    ...
    u8               event_count;                             //   1
    u8               _pad[3];                                 //   3
    DetectionEvent   events[kMaxQueuedEvents];                // 192 (16 × 12)
};
...
public:
    ...
    //   ctx.heartbeatHandler().detectionReport().report(flag, extra);
